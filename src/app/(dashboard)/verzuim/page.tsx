  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {filteredRegistraties.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-12"><Thermometer className="h-12 w-12 text-muted-foreground mb-4" /><h3 className="text-lg font-medium mb-1">Geen verzuimregistraties</h3><p className="text-sm text-muted-foreground mb-4">{activeTab === "actief" ? "Niemand is momenteel ziek gemeld." : "Er zijn nog geen verzuimregistraties."}</p></CardContent></Card>
      ) : (
          ...
          <TableBody>{filteredRegistraties.map((reg) => (
            <TableRow key={reg._id}>
              ...
              <TableCell className="text-right"><div className="flex justify-end gap-1">
                ...
                {isAdmin && (<Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget(reg._id as Id<"verzuimregistraties">)} title="Verwijderen">{actionLoading === reg._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</Button>)}
              </div></TableCell>
