 ...
 * 2. `klantId` reist door álle drie de paden. Vanuit een klantdossier ging de
 *    klant eerder verloren op de vrije route (`?klantId=` werd daar nooit
 *    gelezen). En omgekeerd: zodra de ingang sluit moet de klantcontext weg
 *    zijn, anders start de volgende ⌘N ongemerkt een offerte voor de vorige
 *    klant.
 */
...
const openMenu = async (gebruiker: ReturnType<typeof userEvent.setup>) => {
  ...
  await screen.findByRole("menuitem", { name: /Vrije offerte/ });
};
...
describe("Split-button Nieuwe offerte", () => {
  ...
  it("toont in het chevron-menu drie rijke rijen met sneltoetsen", async () => {
    const gebruiker = userEvent.setup();
    opzet();
    ...
    expect(rijen[0]).toHaveTextContent("Vrije offerte");
    expect(rijen[0]).toHaveTextContent("Meteen een leeg document");
    expect(rijen[0]).toHaveTextContent("V");
    expect(rijen[1]).toHaveTextContent("Scopes kiezen");
    expect(rijen[1]).toHaveTextContent("S");
    expect(rijen[2]).toHaveTextContent("Templates");
    expect(rijen[2]).toHaveTextContent("T");
  });

  it("maakt via 'Vrije offerte' meteen een leeg concept en gaat naar de editor", async () => {
    const gebruiker = userEvent.setup();
    opzet();
    await openMenu(gebruiker);

    await gebruiker.click(screen.getByRole("menuitem", { name: /Vrije offerte/ }));
...
describe("klantId-doorgifte vanaf een klantdossier", () => {
  ...
  it("geeft de klant mee aan de vrije offerte", async () => {
    const gebruiker = userEvent.setup();
    opzet({ klantId: KLANT_ID });
    await openMenu(gebruiker);
    ...
    await waitFor(() =>
      expect(createOfferte).toHaveBeenCalledWith(
        expect.objectContaining({ klantId: KLANT_ID, bron: "vrij" })
      )
    );
  });
